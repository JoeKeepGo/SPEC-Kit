...
For Light work, this kernel is sufficient when current project instructions
cover the task. For Standard or Heavy work, and for acceptance, review,
corrective, or release work, load `skills/sage-kit/SKILL.md` once for the
active task. Descendants inherit the same or narrower boundary; pass that
boundary with delegated work instead of causing nested Skill reloads or
duplicate reviews.

The first progress update after activation emits the non-persistent
`SAGE_ACTIVE` routing marker defined by
`docs/agent/CLAIM_EVIDENCE_TRUST.md`. It is not execution, safety, permission,
or compliance proof and must not be written to project documents, memory,
receipts, or ledgers. Explicit `$sage-kit` invocation remains an override or
diagnostic, and is a fallback only on hosts with neither automatic project
