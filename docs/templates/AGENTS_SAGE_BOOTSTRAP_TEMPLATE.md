...
This project adopts SAGE-Kit. Current authority: `<current-authority>`.
Apply only this lightweight kernel:
...
`references/codex.md` and continue with model-native engineering capabilities.
...
`<installed-sage-kit-path>/SKILL.md` once per controller context. Reload after
compaction or resume when the prior Skill content is no longer observable.
Descendants inherit the same or narrower boundary. Handoffs remain compact:
carry the boundary, selected profile identity, authority references, and, when
complete Skill content is required, the package identity and selected resource
digest, but never the activation marker. Do not cause nested Skill reloads or
duplicate reviews.
