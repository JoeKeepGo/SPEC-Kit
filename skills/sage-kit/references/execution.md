...
Automatic project instructions may carry Light execution with only the
lightweight kernel. This profile is loaded through the complete Skill for
Standard or Heavy execution; load it once for the active task.
...
- Keep subagent boundaries explicit and inherited. Pass descendants the same
  or narrower SAGE boundary and relevant profile without nested Skill reloads
  or duplicate review.
